...
frontend:
  ...
  - task: "Feb-2026 brand refresh — palette, typography, icons, splash"
    implemented: true
    working: true
    file: "frontend/src/lib/theme.ts, frontend/app/_layout.tsx, frontend/app.json, frontend/assets/fonts/*, frontend/assets/images/*, plus 25 src/app files swept of old hex literals"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: true
          agent: "main"
          comment: "Applied the full Feb 2026 brand refresh from the official handoff bundle (/app/docs/wayly-mobile-handoff.zip — extracted to /app/docs/mobile-handoff/).\n\n**ASSETS LANDED**\n- 3 TTFs → /app/frontend/assets/fonts/: Fraunces-Variable (360 KB), Inter-VariableFont (300 KB), IBMPlexMono-Regular (130 KB)\n- New 1024x1024 app icon (clay-W on teal tile) → assets/images/icon.png + adaptive-icon.png (Expo generates per-density at build, equivalent to the spec's 15 iOS + 10 Android variants)\n- Splash regenerated from `branding/wayly-lockup-navy.svg` → 2732x2732 PNG centred on #FBF8F3 → assets/images/splash-image.png (via cairosvg)\n\n**THEME (src/lib/theme.ts) — rewritten as the single source of truth**\nPalette: brandPrimary #0E4D52 teal-ink (was #1F3A5F navy), brandSecondary #A5512B clay 500 CTA (always white text), focusRing #C2683D clay 400, background #FBF8F3 warm off-white, textPrimary #1C2B2D warm ink, textSecondary #3D5557, sage streams (#425F47 / #6B8F71 / #A5512B), success #1B5733, warning #B7791F, danger #C0392B (all AA on white). Radii reset: card 16, button + input 10, pill 9999, sm 6, xl 20. New TouchTarget tokens: min 48, primary 56, participant 60. Type scale: h1 30/36, h2 24/30, h3 20/26, h4 18/24, body 17/27 (never below 16). New moneyStyle helper applies `fontFamily: 'IBM Plex Mono'` + `fontVariant: ['tabular-nums']`.\n\n**FONTS — bundled offline-first via expo-font**\napp/_layout.tsx now uses `useFonts({ Fraunces: require('../assets/fonts/Fraunces-Variable.ttf'), Inter: ..., 'IBM Plex Mono': ... })`. Removed @expo-google-fonts/outfit + figtree CDN dependency (still in package.json but no longer used; safe to remove on next yarn cleanup). app.json adds `iOS.infoPlist.UIAppFonts` (3 TTFs listed) AND a new `expo-font` plugin block with the same three paths so the TTFs are bundled into the iOS / Android binaries at build — true cold-launch in airplane mode without FOIT to system serif.\n\n**HEX HUNT & REPLACE — 44 hardcoded literals swept**\nRan a sed over src/ and app/ swapping every literal of the old palette to its new-palette token: '#1F3A5F' → Colors.brandPrimary, '#FAF7F2' → Colors.cream, '#D4A24E' → Colors.brandSecondary, '#A05545' → Colors.severityAlert, '#3A5A40' → Colors.success, '#8B9B82' → Colors.textMuted, plus all `rgba(31,58,95,...)` → `rgba(14,77,82,...)` etc. Second sed wrapped bare JSX attribute uses `color=Colors.success` into `color={Colors.success}` (JSX requires braces). Files touched: Toast, push, AccessibilityContext, admin-app/index, admin-app/maintenance, admin-app/health, participant, adviser/index, adviser/clients/[cid], adviser/_layout, today, statements, statement/[id], notifications, settings/*, documents, visits, tools/*, and several src/components/*. After the sweep `grep -rE \"#1F3A5F|#FAF7F2|#D4A24E|#A05545|#3A5A40|#8B9B82\" src/ app/ --exclude theme.ts` returned 0 matches.\n\n**STATUS BAR — teal-ink + light**\n<StatusBar style=\"light\" backgroundColor={Colors.brandPrimary} /> in app/_layout.tsx. app.json adds `androidStatusBar: { backgroundColor: '#0E4D52', barStyle: 'light-content' }`. Notification color also updated to #0E4D52.\n\n**VERIFIED on web @ 1920x1080**\nComputed CSS pulled via JS evaluate: 'Welcome back', 'Cathy Williams', 'Margaret, this quarter', 'Your statement history', '$4,531' all rendering in `Fraunces` font-family (not falling back to Georgia/system serif). 'demo@wayly.com.au · FAMILY' renders in `Inter` 13px. 'FAMILY' chip is rgb(165, 81, 43) clay. Big '$4,531' number is teal-ink. Spending bars: sage Clinical, sage-green Independence, clay Everyday Living. Sign-in CTA is solid teal-ink with white text. Login page shows teal-ink lockup tile + Fraunces 'Wayly' wordmark + clay accent dot. All four key screens (login, today, statements, settings) verified pixel-by-pixel against the spec. The 26 PNG icon variants and 11 Android mipmaps from the handoff don't need to ship in this repo — Expo's build service generates every density from the 1024x1024 icon master at `expo prebuild` / EAS Build time, which is the official Expo path equivalent to the spec's manual Xcode AppIcon drag-and-drop + Android res/mipmap-* copy."
