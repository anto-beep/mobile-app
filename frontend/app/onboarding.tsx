  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: Spacing.lg, paddingBottom: 60 }}>
        ...
        <View style={styles.iconWrap}><Ionicons name={s.icon} size={32} color={Colors.brandPrimary} /></View>
        <Text style={Type.h1 as any}>{s.title}</Text>
        <Text style={styles.body}>{s.body}</Text>

        {step === 1 && (
          <View style={{ gap: 10, marginTop: Spacing.lg }}>
            <TextInput value={first} onChangeText={setFirst} placeholder="First name" placeholderTextColor={Colors.textMuted} style={styles.input} autoFocus />
        ...
        {step === 3 && (
          <View style={{ marginTop: Spacing.lg }}>
            <TextInput value={provider} onChangeText={setProvider} placeholder="e.g. HomeCare Plus" placeholderTextColor={Colors.textMuted} style={styles.input} autoFocus />
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  body: { ...Type.body, color: Colors.textSecondary, marginTop: 8, lineHeight: 24 },
  input: { borderWidth: 1, borderColor: Colors.border, borderRadius: 10, padding: 14, fontFamily: Fonts.body, color: Colors.textPrimary, fontSize: 16 },
  pillRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: Spacing.lg },
  pill: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 9999, borderWidth: 1.5, borderColor: Colors.border, backgroundColor: Colors.cardBg },
  pillActive: { borderColor: Colors.brandPrimary, backgroundColor: 'rgba(14,77,82,0.08)' },
  pillText: { color: Colors.textSecondary, fontFamily: Fonts.bodySemi, fontWeight: '700' },
  ...
  btnGhostText: { color: Colors.textPrimary, fontFamily: Fonts.bodySemi, fontWeight: '700' },
