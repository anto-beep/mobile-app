    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={styles.input} value={participant} onChangeText={setParticipant} placeholder="e.g. Margaret" placeholderTextColor={Colors.textMuted} testID="reassess-participant" />
          ...
          {letterType === 'rcp_assessment' && (
            <View testID="rl-rcp-fields">
              ...
              <TextInput style={styles.input} value={hospital} onChangeText={setHospital} placeholder="e.g. Royal Melbourne Hospital" placeholderTextColor={Colors.textMuted} testID="rl-hospital" />
              <Text style={styles.label}>Discharge date</Text>
              <TextInput style={styles.input} value={discharge} onChangeText={setDischarge} placeholder="YYYY-MM-DD" placeholderTextColor={Colors.textMuted} testID="rl-discharge" />
            </View>
          )}

          <Text style={styles.label}>What's changed?</Text>
          <TextInput style={[styles.input, { minHeight: 100, textAlignVertical: 'top' }]} value={changes} onChangeText={setChanges} placeholder="e.g. New mobility issues since fall in March; increased confusion in the evenings; can no longer prepare meals safely…" placeholderTextColor={Colors.textMuted} multiline testID="reassess-changes" />

          <Text style={styles.label}>Recent events (optional)</Text>
          <TextInput style={[styles.input, { minHeight: 80, textAlignVertical: 'top' }]} value={events} onChangeText={setEvents} placeholder="e.g. Hospital admission in February" placeholderTextColor={Colors.textMuted} multiline testID="reassess-events" />

          <Text style={styles.label}>Your name</Text>
          <TextInput style={styles.input} value={sender} onChangeText={setSender} placeholder="Your full name" placeholderTextColor={Colors.textMuted} testID="reassess-sender" />
...
const styles = StyleSheet.create({
  ...
  typeGrid: { gap: 6 },
  typeCard: { padding: 10, borderRadius: Radius.md, borderWidth: 1, borderColor: Colors.borderSubtle, backgroundColor: Colors.cardBg },
  typeCardOn: { borderColor: Colors.brandPrimary, backgroundColor: 'rgba(14, 77, 82, 0.06)' },
  typeLabel: { fontFamily: Fonts.bodySemi, fontSize: 14, color: Colors.textPrimary },
  typeLabelOn: { color: Colors.brandPrimary },
  typeSub: { fontFamily: Fonts.body, fontSize: 11, color: Colors.textMuted, marginTop: 2, lineHeight: 15 },
  ...
  resultTitle: { fontFamily: Fonts.headingMed, fontSize: 16, color: Colors.brandPrimary, flex: 1 },
